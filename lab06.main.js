...
healthcheck(callback) {
 this.getRecord((result, error) => {
   ...
   if (error) {
      ...
      */
      this.emitOffline();
      log.error(`Healthcheck error @id[${this.id}]: ${JSON.stringify(error)}`);
      if (callback) {
      ...
      */
      this.emitOnline();
      log.debug(`Healthcheck success @id[${this.id}]`)
      if (callback) {
          return callback(result, error)
      }
   }
  ...
  getRecord(callback) {
     ...
     */
     this.connector.get((responseData, errorMessage) => callback(responseData, errorMessage));
  }
